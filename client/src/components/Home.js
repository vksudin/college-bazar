...
export default function CallToActionWithIllustration() {
    ...
    const getData=()=>{
        ...
        axios.get('/api/private/getitems',{
            headers:{
                "Content-Type":"application/json",
                "Authorization":`Bearer ${localStorage.getItem("clgToken")}`
            }
    ...
    const fetchItems=(cats,limit)=>{
        ...
        axios.post('/api/private/getfiltereditems',{categories:arr,limit},{
            headers:{
                "Content-Type":"application/json",
                "Authorization":`Bearer ${localStorage.getItem("clgToken")}`
            }
    ...
    const searchItem=async(val)=>{
        try {
            const {data}=await axios.post('/api/private/getsearchresults',{field:val},{
                headers:{
                    "Content-Type":"application/json",
                    "Authorization":`Bearer ${localStorage.getItem("clgToken")}`
                }
        ...
        >
          ...
          >
            ...
            <Text color={"gray.500"} maxW={"3xl"}>
              <strong>
                Do you want to buy college notes, books and furnitures used by
                others at reasonable price? Then you are at a perfect place. You
                can also post and sell your items through College Bazar
              </strong>
            </Text>
            ...
            <Flex w={"full"}>
              <img
                src="https://penji.co/wp-content/uploads/2021/02/higher-education-marketing.jpg"
                alt="college-img"
                style={{ width: "100vw" }}
              />
            </Flex>
          ...
          <Center height="50px" flexDir={"row"} flexWrap={"wrap"}>
            <Stack
              direction="row"
              height="38px"
              align={"center"}
                ...
                categories.map((e, i) => {
                  return (
                    <Badge
                      cursor="pointer"
                      onClick={async () => {
                        if (catArr.includes(e)) {
                          setCatArr(() => {
                            return catArr.filter((curr) => e._id !== curr._id);
                          });
                          let temp = catArr.filter(
                            (curr) => e._id !== curr._id
                          );
                          console.log(
                            catArr.filter((curr) => e._id !== curr._id)
                          );
                          fetchItems(temp, range);
                        } else {
                          setCatArr([...catArr, e]);
                          console.log([...catArr, e]);
                          fetchItems([...catArr, e], range);
                        }
                      }}
                      colorScheme={catArr.includes(e) ? "green" : "red"}
                      key={i}
                    >
                      {e.categoryname}
                    </Badge>
            ...
            bg='white.300'
              ...
              onChange={(e) => {
                ...
                fetchItems(catArr, e.target.value);
              }}
            >
          ...
          <SimpleGrid columns={[1, 1, 2, 3]} spacing="1px">
            {items.length > 0
              ? items.map((e, i) => {
                  return (
                    <Card
